...
// defaults as the Python implementation.
...
func NewLambdaComponent(ctx *pulumi.Context, name string, args *LambdaComponentArgs, opts ...pulumi.ResourceOption) (*LambdaComponent, error) {
	...
	err := ctx.RegisterComponentResource("oneblood:lambda:LambdaComponent", name, comp, opts...)
	if err != nil {
		return nil, err
	}
	...
	if cfg.AlertsEmail != nil && *cfg.AlertsEmail != "" {
		_, err = cloudwatch.NewCloudwatchComponent(ctx, fmt.Sprintf("%s-cw", name), &cloudwatch.Args{
			Function:         fn,
